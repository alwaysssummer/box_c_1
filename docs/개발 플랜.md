...
Phase 20: 블록 기반 문제 출제 시스템 ────────────── 3일  ✅ 완료
Phase 21: 문제 유형 정의 개선 ─────────────────── 1일  ✅ 완료
Phase 22: 출력 설정 시스템 ────────────────────── 7일  📋 계획 수립
Phase 23: 문제관리 UI/UX 개선 ─────────────────── 1일  ✅ 완료
                                    ─────────────
                                    총 약 41일
```
...
### Task 7.4: API 엔드포인트 ✅ 완료
- [x] `/api/passages/batch-delete-generated` - 일괄 삭제 (전체/유형별)
- [x] `/api/passages/[id]/generated` - 지문 상세 조회
- [x] `/api/generated-data/[id]` - 개별 데이터 조회/삭제
- [x] `/api/generated-questions/[id]` - 개별 문제 조회/삭제
...
| Phase 7.5: 문제 관리 (완전 구현) | ✅ 완료 | 2024-12-12 |
| Phase 8: PDF 출력 | ⏳ 대기 | - |
| Phase 9: 테스트/배포 | 🔄 진행중 | - |
...
| **Phase 20: 블록 기반 시스템** | ✅ 완료 | 2024-12-11 |
| **Phase 21: 문제 유형 개선** | ✅ 완료 | 2024-12-11 |
| **Phase 22: 출력 설정** | 📋 계획 수립 | - |
| **Phase 23: 문제관리 UI/UX 개선** | ✅ 완료 | 2024-12-12 |
...
| 날짜 | 내용 |
|------|------|
| 2024-12-12 | **🎨 문제관리 UI/UX 개선**: 필터를 중앙 패널 상단으로 이동, 전체 선택/해제 기능 추가, 카드 스타일 적용 (문제출제와 UX 통일) |
| 2024-12-12 | **✅ 문제관리 기능 완전 구현**: 일괄삭제 API 4개 구현, 지문 상세 패널, 문제 미리보기 모달 연결 완료 |
| 2024-12-11 | **🔧 출력설정 플랜 개선**: 외부 분석 검토 후 프리셋 기반 설계로 단순화, 3 Phase로 재구성 (22-A/B/C) |
| 2024-12-11 | **📄 출력설정 개발 플랜 작성**: 18개 잠재적 문제점 분석, OutputConfig 데이터 구조 설계 (`docs/출력설정개발플랜.md`) |
| 2024-12-11 | **🔄 문제유형 위자드 통합**: 4단계 → 2단계 (기본 설정 / 레이아웃 & 뷰) |
| 2024-12-11 | **✅ 블록 기반 문제 출제 시스템 구현**: BlockDefinition, BlockInstance, modifies_passage 플래그 |
| 2024-12-11 | **🔍 AI 프롬프트 개선 기능**: 자연어 지시로 프롬프트 개선, 자동 테스트 재실행 |
| 2024-12-11 | **👁 블록 미리보기 기능**: 필드 적용 전 실제 출력물 확인 모달 |
...
# 🧱 블록 기반 문제 출제 시스템 (2024-12-11)

## Phase 20: 블록 기반 문제 출제 시스템 ✅ 완료
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  1. 블록 정의 (BlockDefinition)                                 │
│     └── 프롬프트 + 출력 필드 + 지문 가공 여부                   │
│                              ↓                                  │
│  2. 블록 인스턴스 (BlockInstance)                               │
│     └── 지문 + 블록정의 → AI 생성 결과                          │
│                              ↓                                  │
│  3. 문제 유형 (QuestionType)                                    │
│     └── 블록 선택 + 레이아웃 + 뷰 설정                          │
│                              ↓                                  │
│  4. 최종 문제 (GeneratedQuestion)                               │
...
| # | 기능 | 파일 | 상태 |
|---|------|------|:----:|
| 1 | BlockDefinition 타입/테이블 | `types/database.ts`, DB | ✅ |
| 2 | BlockInstance 타입/테이블 | `types/database.ts`, DB | ✅ |
| 3 | 블록 관리 UI (BlockForm) | `components/features/block/BlockForm.tsx` | ✅ |
| 4 | 블록 생성 API | `/api/block-definitions/route.ts` | ✅ |
| 5 | 문제 생성 API (블록 기반) | `/api/generate/route.ts` | ✅ |
| 6 | 문제 등록 API | `/api/generate/register/route.ts` | ✅ |
| 7 | modifies_passage 플래그 | DB 컬럼, UI 체크박스 | ✅ |
| 8 | AI 프롬프트 개선 기능 | `/api/block-definitions/improve/route.ts` | ✅ |
| 9 | 블록 테스트 기능 | 지문 선택 → AI 테스트 → 필드 감지 | ✅ |
| 10 | 블록 미리보기 기능 | 모달에서 실제 출력물 확인 | ✅ |
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  modifies_passage = false (기본)                                │
│  └── AI 출력에서 passage/body 무시                              │
│  └── 원본 지문을 그대로 사용 (환각 방지)                         │
│                                                                 │
│  modifies_passage = true                                        │
│  └── AI 출력에서 passage/body 사용                              │
│  └── 빈칸추론, 순서배열 등 지문 가공 유형에 적합                 │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
```sql
-- supabase/migrations/20241211_add_modifies_passage.sql
...
│                    Before (4단계)                               │
├─────────────────────────────────────────────────────────────────┤
│  Step 1: 출력 유형 (문제형/자습서형)                             │
│  Step 2: 블록 선택                                               │
│  Step 3: 레이아웃 (배치모드, 컬럼, 선택지 스타일)                │
│  Step 4: 출력 뷰 (학생/정답/교사용 필드)                         │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│                    After (2단계)                                │
...
| 파일 | 변경 내용 |
|------|----------|
| `QuestionTypeForm.tsx` | WIZARD_STEPS 4개 → 2개, UI 통합 |

---

## Phase 22: 출력 설정 시스템 📋 계획 수립 (개선)

### 📄 상세 개발 문서

> **참조**: `docs/출력설정개발플랜.md`

### 🔍 외부 분석 검토 결과 (2024-12-11)

외부 AI(Claude/OpenAI/Gemini) 분석을 검토한 결과, 우리 프로젝트 맥락에서의 적용 여부를 판단했습니다.

#### ✅ 채택 항목

| 제안 | 이유 | 적용 방식 |
|------|------|----------|
| **프리셋 기반 설계** | 30개+ 설정 → 5개 프리셋으로 단순화 | 프리셋 상수 + 오버라이드 |
| **필드 정규화 함수** | AI 응답 타입 불일치 실제 발생 | 저장 시점 정규화 |
| **CSS 프리셋 클래스** | 레이아웃 일관성 확보 | 프리셋별 CSS 클래스 |

#### ❌ 불채택 항목

| 제안 | 불채택 이유 |
|------|------------|
| SourceStrategy | `modifies_passage` 플래그로 이미 해결됨 |
| 테마 시스템 분리 | 1인 관리자 시스템에서 불필요한 복잡도 |
| Two-Layer Rendering | 현재 규모에서 과도한 아키텍처 |
| DB 분리 (passage/question) | 기존 구조로 충분 |
| Atomic Unit | 실제 문제 발생 시 검토 (현재 불필요) |

#### ⚠️ 이미 보유한 기능

| 외부 제안 | 우리가 가진 것 |
|----------|---------------|
| "뷰별 필드 제한 필요" | `LayoutConfig.views` |
| "필드 렌더러 구축" | `QuestionRenderer` |
| "PDF 시스템 구축" | 강사 페이지 `html2pdf.js` |

---

### 핵심 원칙

> **"80%는 프리셋으로, 20%만 커스터마이징"**

기존 `LayoutConfig` 구조를 유지하면서 프리셋 선택으로 대부분 자동 설정
...
|-------|------|----------|:----:|
| **22-A** | 안전장치 + 프리셋 | 2일 | ⏳ 대기 |
| **22-B** | 레이아웃 + 미리보기 | 3일 | ⏳ 대기 |
| **22-C** | 고급 기능 (선택) | 향후 | ⏳ 대기 |

#### Phase 22-A: 안전장치 + 프리셋 (2일)

```
Day 1:
├── 필드 검증 함수 (validateFields)
├── 필드 정규화 함수 (normalizeBlockOutput)
└── 뷰별 필드 제한 (V-01 정답노출 방지)

Day 2:
├── 5개 프리셋 정의 (LAYOUT_PRESETS 상수)
├── 프리셋 선택 UI (드롭다운)
└── 기존 LayoutConfig → 프리셋 자동 매핑
```

#### Phase 22-B: 레이아웃 + 미리보기 (3일)

```
Day 3-4:
├── 프리셋별 CSS 클래스 (.layout-csat, .layout-school 등)
├── 오버플로우 처리 (미리보기: 스크롤, 출력: 자동분할)
└── 기존 QuestionRenderer 확장
...
#### Phase 22-C: 고급 기능 (선택, 향후)

```
선택적 구현 (필요 시):
├── 필드 순서 드래그&드롭
├── 워터마크
└── 페이지 분할 고도화 (Atomic Unit - 실제 문제 발생 시)
```
...
| 프리셋 | 설명 | 용도 | 기본 설정 |
|--------|------|------|----------|
| `csat` | 수능 2단형 | P1-P6, T1-T3 | 2단, 지문좌측 |
| `school_top` | 내신 지문상단 | T6-T9 | 1단, 지문상단 |
| `school_multi` | 내신 다문제 | 1지문 다문제 | 1단, 지문상단고정 |
| `study` | 자습서 1단형 | L1-L4 | 1단, 연속흐름 |
| `sentence` | 문장-문제 쌍 | S1-S3 | 1단, 쌍반복 |

### 프리셋 데이터 구조 (단순화)

```typescript
// 기존 LayoutConfig 활용 + 프리셋 추가
...
interface QuestionTypeFormData {
  // 기존 필드들...
  layout_preset: keyof typeof LAYOUT_PRESETS  // 프리셋 선택
  layout_overrides?: Partial<LayoutConfig>     // 필요시 오버라이드
}
```
...
export function normalizeBlockOutput(raw: Record<string, unknown>): Record<string, unknown> {
  return {
    ...raw,
    // choices: 배열 강제
    choices: Array.isArray(raw.choices) 
      ...
      : typeof raw.choices === 'string' 
        ? parseChoicesString(raw.choices)
        : [],
    // answer: 문자열 강제
    answer: raw.answer != null ? String(raw.answer) : '',
    // explanation: 문자열 강제, 없으면 빈 문자열
    explanation: typeof raw.explanation === 'string' ? raw.explanation : '',
  }
}

function parseChoicesString(str: string): string[] {
  // ①②③ 또는 1.2.3. 패턴 파싱
  const circled = str.match(/[①②③④⑤][^①②③④⑤]*/g)
  if (circled) return circled.map(s => s.slice(1).trim())
  
  const numbered = str.split(/\d+\.\s*/).filter(Boolean)
...
1. **Phase 22-A 착수**: 프리셋 상수 정의 + 정규화 함수 구현
2. **문제 유형 폼 연동**: 프리셋 선택 드롭다운 추가
3. **Phase 22-B**: 레이아웃 CSS + 미리보기 개선

---

## 📌 전체 진행 현황 (2024-12-11 기준)

| Phase | 상태 | 완료일 |
|-------|------|--------|
| Phase 0-9: MVP 1.0 | ✅ 완료 | 2024-12-05 |
| Phase 10-14: 출제 시스템 | ✅ 완료 | 2024-12-06 |
| Phase 15-19: 원큐 시스템 고도화 | ✅ 완료 | 2024-12-06 |
| **Phase 20: 블록 기반 시스템** | ✅ 완료 | 2024-12-11 |
| **Phase 21: 문제 유형 개선** | ✅ 완료 | 2024-12-11 |
| **Phase 22: 출력 설정** | 📋 계획 개선 완료 | - |
