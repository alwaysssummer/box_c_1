...
### Phase 15: 원큐 UI 단순화 ─────────────────── 1일 🔄 진행중

| 작업 | 상태 | 설명 |
|------|:----:|------|
| 입구 분리 (원큐/슬롯) | ✅ | QuestionTypeModeSelector 구현 |
| 프롬프트 원큐 폼 | ✅ | PromptBasedForm 구현 (5개 필드) |
| 폼 간소화 | ⏳ | 3개 필드로 축소 (유형명, 프롬프트, 레이아웃) |
| 기존 슬롯 폼 분리 | ✅ | QuestionTypeFormNew 유지 |

### Phase 16: 그룹별 레이아웃 시스템 ───────────── 2일 ⏳ 대기

| 작업 | 상태 | 설명 |
|------|:----:|------|
| 레이아웃 서브타입 정의 | ⏳ | GROUP_LAYOUTS 상수 추가 |
| practical 레이아웃 | ⏳ | 표준형, 박스형, 빈칸형 |
| selection 레이아웃 | ⏳ | 양자택일, 밑줄형 |
| analysis 레이아웃 | ⏳ | 세로형, 2열형 |
| writing 레이아웃 | ⏳ | 배열형, 영작형 |

### Phase 17: 파싱-렌더링 일관성 ────────────── 2일 ⏳ 대기

| 작업 | 상태 | 설명 |
|------|:----:|------|
| 출력포맷 템플릿 검증 | ⏳ | OUTPUT_FORMAT_TEMPLATES 점검 |
| 파싱 로직 고도화 | ⏳ | parsePromptResult 개선 |
| QuestionRenderer 확장 | ⏳ | 레이아웃 서브타입 분기 렌더링 |
| 테스트 케이스 | ⏳ | 그룹별 파싱/렌더링 테스트 |

### Phase 18: 프롬프트 라이브러리 구축 ──────── 3일 ⏳ 대기
...
| L1~L4 (학습자료) | ⏳ | 분석형 자료 |
| 프롬프트 검증/확정 | ⏳ | 각 프롬프트 테스트 후 status: confirmed |

### Phase 19: 강사 페이지 출력 완성 ─────────── 2일 ⏳ 대기

| 작업 | 상태 | 설명 |
|------|:----:|------|
| 레이아웃 선택 UI | ⏳ | 그룹별 레이아웃 옵션 표시 |
| A4 페이지네이션 고도화 | ✅ | 100% 정확한 페이지 분할 |
| 밀도 옵션 | ✅ | compact/normal/spacious |
| 인쇄 스타일 | ✅ | CSS @media print |
| PDF 생성 | ⏳ | html2pdf 또는 react-to-print |
...
```
Phase 15: UI 단순화 ─────────────── 1일  🔄 진행중
Phase 16: 레이아웃 시스템 ─────────── 2일  ⏳ 대기
Phase 17: 파싱-렌더링 일관성 ──────── 2일  ⏳ 대기
Phase 18: 프롬프트 라이브러리 ─────── 3일  ⏳ 대기
Phase 19: 출력 완성 ─────────────── 2일  ⏳ 대기
                        ─────────────
                        총 약 10일
```
